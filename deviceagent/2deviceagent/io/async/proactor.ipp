...
template <typename config>
void proactor<config>::start_write_op(op_poll* op, object_impl& impl,
	bool is_nonblocking) {
...
template <typename config>
void proactor<config>::start_read_op(op_poll* op, object_impl& impl,
	bool is_nonblocking) {
...
template <typename config>
void proactor<config>::start_connect_op(op_poll* op, object_impl& impl,
	bool is_nonblocking) {
...
template <typename config>
int proactor<config>::run(int& ec) {
	return io_service_->run(ec);
}

template <typename config>
void proactor<config>::post_immediate_completion(op_poll* op) {
	io_service_->post_immediate_completion(op);
}

template <typename config>
void proactor<config>::post(post_handler& h) {
	io_service_->post(h);
}
...
template <typename config>
void proactor<config>::cancel_ops(object_impl& impl) {
	reactor_->cancel_ops(impl.get_descriptor(), impl.get_reactor_data());
}
